...
export default function Login() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session && !hasRedirected.current) {
      ...
      } else if (!backendToken && (session.user as any)?.email) {
        // Fallback for NextAuth session if backendToken is missing
        console.warn("Session authenticated but backend token missing");
      }
    }
